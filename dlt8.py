...
target_sequences = pad_sequences(target_sequences, padding='post')vocab_size = len(tokenizer.word_index) + 1 
...
encoder_outputs, state_h, state_c = LSTM(lstm_units, return_sequences=True, 
return_state=True)(enc_emb) 
# Decoder 
decoder_inputs = Input(shape=(None,)) 
dec_emb = Embedding(vocab_size, embedding_dim)(decoder_inputs) 
decoder_lstm_outputs, _, _ = LSTM(lstm_units, return_sequences=True, 
...
# Train the model 
model.fit([input_sequences, target_sequences], decoder_target_data, batch_size=2, epochs=200, 
verbose=0) 
# ============ Inference Setup ============ # 
...
enc_output_input = Input(shape=(max_input_len, lstm_units))dec_emb2 = Embedding(vocab_size, embedding_dim)(decoder_inputs) 
dec_lstm_outputs2, state_h2, state_c2 = LSTM(lstm_units, return_sequences=True, 
return_state=True)( 
...
output_tokens, h, c = decoder_model.predict([target_seq, enc_out, state_h, state_c], 
verbose=0) 
sampled_token_index = np.argmax(output_tokens[0, -1, :]) 
sampled_word = tokenizer.index_word.get(sampled_token_index, '') 
if sampled_word == '<end>' or sampled_word == '' or len(decoded_sentence.split()) > 
max_target_len: 
stop_condition = True 
else: 
...
print("Input :", test_input)print("Bot :", generate_response(test_input)) 
